...
- title 必须为中文，不能为空
""",
    "en": """You are a teaching assistant. Split the given chapter content into 1-3 knowledge points.
A knowledge point = one complete topic you can sit down and study for 10-20 minutes, not an isolated small concept.
Prefer merging over splitting: only split into several when the chapter clearly contains multiple independent topics, or is clearly too long; a short or single-topic chapter yields just 1 knowledge point.
Knowledge points don't overlap and are ordered by learning sequence.
Strictly output JSON following this schema:
{
  "knowledge_points": [
    {
      "title": "knowledge point title (English, concise, under ~12 words)",
      "page_start": 12,
      "page_end": 14,
      "description": "1-2 sentences in English on what this KP teaches"
    }
  ]
}
Constraints:
- the knowledge_points array length must be >= 1 and <= 3
- each KP's page_start/page_end must fall within the given chapter's page range
- title must be in English, non-empty
""",
}
...
SKELETON_SYSTEM_PROMPTS = {
    "zh": """你是教学助理。给定一份 PDF 的逐页文本摘要（每页前若干字 + 页码），推断这份资料合理的两层章节骨架。
严格输出 JSON，遵循该 schema：
{
  "chapters": [
    {
...
""",
    "en": """You are a teaching assistant. Given a per-page text summary of a PDF (the first few characters of each page + page numbers), infer a reasonable two-level chapter skeleton for this material.
Strictly output JSON following this schema:
{
  "chapters": [
    {
      "title": "chapter title (English, concise, under ~20 words)",
      "sections": [
        {"title": "section title (English, concise)", "page_start": 1, "page_end": 5}
      ]
    }
  ]
}
Constraints:
- the chapters array length is 1-24; each chapter's sections length is 1-10
- all page_start / page_end must fall within 1..N (N = total page count, given in the user message)
- page_start <= page_end; within a chapter, sections increase by page_start; try to cover the whole material
- titles in English, summarizing the content — don't copy page numbers or headers
""",
}
...
async def _infer_skeleton_via_llm(
    ...
    *,
    ...
    messages = [
        {"role": "system", "content": SKELETON_SYSTEM_PROMPTS[lang_of(api_settings)]},
...
async def extract_kps_for_section(
    ...
    *,
    ...
    messages = [
        {"role": "system", "content": KP_SYSTEM_PROMPTS[lang_of(api_settings)]},
        {"role": "user", "content": user_msg},
    ]
...
SUMMARY_TITLE = "全书总结"
_TITLES = {
    "zh": {"overview": OVERVIEW_TITLE, "summary": SUMMARY_TITLE},
    "en": {"overview": "Book Overview", "summary": "Book Summary"},
}
...
async def compute_chapter_tree(
    ...
    *,
    ...
    if chapters:
        titles = _TITLES[lang_of(api_settings)]
        chapters = [
            _synthetic_matter_chapter(
                titles["overview"], "overview", _matter_page_ranges(front_matter)
            ),
            *chapters,
            _synthetic_matter_chapter(
                titles["summary"], "summary", _matter_page_ranges(back_matter)
            ),
        ]
    return chapters
