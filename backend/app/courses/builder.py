...
KP_SYSTEM_PROMPT = """你是教学助理。把给定章节内容切分为 3-7 个知识点。
每个知识点聚焦单一核心概念，互不重叠，按学习顺序排列。
...
class _KPListSchema(BaseModel):
    knowledge_points: list[_KPSchema] = Field(..., min_length=3, max_length=7)
    ...
    async def _extract_one(sec: SectionLayout) -> list[_KPSchema]:
        ...
        async with sema:
            result = await extract_kps_for_section(
                sec, _section_text(page_texts, sec), api_settings=api_settings
            )
