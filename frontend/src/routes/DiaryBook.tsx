...
function authorViewFor(entry: DiaryEntry): AuthorView {
  const label = entry.author_label?.trim() || "老师";
  const sig = entry.author_signature?.trim() || label;
  const stamp = Array.from(label)[0] ?? "师";
...
export function DiaryBook() {
  ...
  useEffect(() => {
    ...
    getCourseDiary(courseId)
      ...
      .catch((err: unknown) => {
        if (cancelled) return;
        const message = err instanceof DiaryError ? err.message : "加载失败";
  ...
  if (state.kind === "loading") {
    return (
      <main className="diary-page-root page-in">
        <div className="margin-note">翻开日记本…</div>
      </main>
    );
  }
  if (state.kind === "error") {
        ...
        <div className="diary-body" style={{ gridTemplateColumns: "1fr" }}>
          <div className="diary-spread">
            <div className="diary-pending">
              ...
              <div className="diary-pending-text">日记本还是空的。</div>
...
function DiaryHead({
  ...
  return (
    <header className="diary-head">
      <div>
        <div className="diary-eyebrow">A Teacher&apos;s Journal · 教师手记</div>
      ...
      <div className="diary-head-meta">
        ...
        <span className="num-unit">篇已成稿</span>
        <div style={{ marginTop: 4 }}>共 {authorCount} 位执笔人</div>
...
function DiaryTOC({
  ...
  return (
    <nav className="diary-toc">
      <div className="diary-toc-title">编年索引</div>
...
function DiaryEntryView({ entry }: { entry: EntryView }) {
  ...
  return (
    <article>
        ...
        <div className="diary-entry-folio">
          {entry.raw.attempt > 1 && (
            <span
              ...
              title={`第 ${entry.raw.attempt} 次执笔`}
            >
              retry · 第 {entry.raw.attempt} 次
            </span>
          )}
          <span className="roman">Folio</span> · 第 {entry.folio} 篇
        </div>
      </div>
      ...
      <div className="diary-entry-kp">关于「{entry.raw.kp_title}」</div>
...
function DiaryPending({ entry }: { entry: EntryView }) {
  ...
  const hint =
    status === "failed"
      ...
      : status === "running"
        ? "今夜尚未提笔——老师还在桌前推敲措辞。"
        : `这一节她还在路上。等这堂课结束，${entry.author.label}会回到桌前，把今晚的事写下来。`;
...
function DiaryMargin({
  ...
  return (
    <aside className="diary-margin">
      <div className="diary-margin-title">边 · 注</div>

      {isPending && (
        <div className="diary-margin-block">
          <div className="diary-margin-key">此刻</div>
          ...
          <div className="diary-margin-pencil">
            等她说出&ldquo;好像懂了&rdquo;，今晚的日记才会开始写。
          </div>
        </div>
      )}
      ...
      <div className="diary-margin-actions">
        ...
        >
          ...
          <span className="diary-margin-link-inner">
            <span className="diary-margin-link-label">返回主页</span>
        ...
        >
          <span className="diary-margin-link-line" aria-hidden />
          <span className="diary-margin-link-inner">
            <span className="diary-margin-link-label">返回该章节</span>
