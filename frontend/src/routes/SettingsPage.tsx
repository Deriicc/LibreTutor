...
export function SettingsPage() {
  ...
  async function onSave() {
    ...
    try {
      ...
      alert(e instanceof SettingsError ? e.message : "保存失败");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function probe(
    ...
    try {
      ...
      setP({
        ...
        result: {
          ok: false,
          detail: e instanceof SettingsError ? e.message : "测试失败",
        },
      });
    }
  }
  ...
  return (
    <main style={wrap} className="page-in">
      ...
      >
        系统不再提供公用 Key —— 对话、评分、评估、日记、课程构建均使用你在此填写的
        API。未配置 Chat 时这些功能不可用。
      </p>

      <section className="card" style={cardStyle}>
        <h2 style={h2}>对话模型（Chat）</h2>

        <label className="label">接口格式</label>
        <select
          className="input"
        ...
        <p className="margin-note" style={hint}>
          两者都通过 OpenAI 兼容客户端调用；选 Anthropic
          时请把 Base URL 指向其 OpenAI 兼容代理。
        </p>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          >
            {chatProbe.kind === "running" ? "测试中…" : "测试连通性"}
          </button>
      ...
      <section className="card" style={cardStyle}>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          >
            {embProbe.kind === "running" ? "测试中…" : "测试连通性"}
          </button>
